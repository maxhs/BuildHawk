class Api::V3::GroupsController < Api::V3::ApiController
    ...
    def show
        ...
        user = User.find params[:user_id]
    	render json: {id: group.id, name: group.name, projects: group.projects.map{|project| {name: project.name, hidden: project.hidden_for_user?(user), address: {formatted_address: project.address.formatted_address} } } }
    end
