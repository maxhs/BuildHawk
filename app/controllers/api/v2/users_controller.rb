class Api::V2::UsersController < Api::V2::ApiController
	...
	def connect
		...
		if @user
			if project
				items = Task.where(:assignee_id => @user.id).map{|t| t if t.tasklist.project.id = project.id}.compact
			else
				items = Task.where(:assignee_id => @user.id).map{|t| t if t.tasklist.project.company.id != @user.company.id}.compact
			end
			respond_to do |format|
