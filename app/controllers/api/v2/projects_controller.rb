class Api::V2::ProjectsController < Api::V2::ApiController
    ...
    def index
        ...
        projects = user.project_users.where("archived = ? and core = ? and project_group_id IS NULL",false,false).map(&:project)#.map{|p| p.project if p.project.company_id == user.company_id}.compact.sort_by{|p| p.name.downcase}
        if projects
